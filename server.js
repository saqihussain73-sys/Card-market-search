...
app.get("/api/retail/card-empire",async(req,res)=>{
 ...
 if(!["pokemon","yugioh"].includes(game)||!product||!set||product.length>180||set.length>120)return res.json({offer:null});
 const key=JSON.stringify([game,product,set,type]), cached=cardEmpireCache.get(key);
 if(cached&&Date.now()-cached.at<CE_TTL)return res.json(cached.data);
 try{
  const query=encodeURIComponent(set+" "+product);
  const response=await fetch("https://www.cardempire.com/search/suggest.json?q="+query+"&resources[type]=product&resources[limit]=10",{headers:{"User-Agent":"CardMarketSearch/1.0"},signal:AbortSignal.timeout(8000)});
  if(!response.ok)throw Error("Card Empire search unavailable");
  const data=await response.json();
  const products=data.resources?.results?.products||[];
  const matches=products.filter(item=>matchCardEmpire(item,game,{product,set,type})).map(item=>{
   const price=Number(item.price);
   const url=new URL(item.url||"", "https://www.cardempire.com");
   if(url.hostname!=="www.cardempire.com"||!url.pathname.startsWith("/products/")||!Number.isFinite(price)||price<=0)return null;
   return {name:item.title,priceGBP:price,url:url.href};
  }).filter(Boolean);
  const result={offer:matches.length===1?matches[0]:null};
  cardEmpireCache.set(key,{at:Date.now(),data:result});
  res.json(result);
 }catch(err){console.warn("Card Empire lookup failed:",err.message);res.json({offer:null});}
});
